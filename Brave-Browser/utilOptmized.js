...
const getNPMConfig = (path) => {
  return getNPMConfigFromEnv(path) || getNPMConfigFromPackageJsonOptmized(path)
}
...
const updateEnvPath = (config) => {
  const envPath = path.resolve(__dirname, '.env.config.project.path')

  if(!envPath) return
  
  const newEnvConfig = `PROJECT_PATH=${config}`
...
module.exports = {
  ...
  run,
  runGit, 
  getNPMConfigFromPackageJsonOptmized
}